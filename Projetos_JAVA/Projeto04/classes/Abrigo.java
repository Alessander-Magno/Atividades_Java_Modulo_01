...
public class Abrigo
{
    ...
    {        
        try(BufferedReader br = new BufferedReader(new FileReader(caminhoAbrigo)))
        {
            String linha;
    ...
    //particionamento da linha do arquivo txt em subpartes menores, ex.: idade: 18
    {
    ...
    public void info(Scanner scan, String nome, String email) //informa todos os animais cadastrados
    {
        for (int i = 0; i < linhasTotais.size(); ++i)
        {
            System.out.printf("ID: %d | %-21s | %-12s | %-30s | %-22s | %s\n", i, nomeList.get(i), idadeList.get(i), tipoList.get(i), racaList.get(i), personList.get(i));
        }
        ...
        if (id >= 0 && id < linhasTotais.size())
        {
            ...
            String pedido = String.format("Remetente: %s, %s, %s, %s, %s, %s", email, nomeList.get(id), idadeList.get(id), tipoList.get(id), racaList.get(id), personList.get(id));
            //System.out.println(pedido);

            //Atribuição das linhas dos pedidos a coleção
            try(BufferedReader br = new BufferedReader(new FileReader(caminhoPedidos)))
            {
                String linha;
            ...
            {
                try(BufferedWriter bw = new BufferedWriter(new FileWriter(caminhoPedidos, true)))
                {
                    bw.write(pedido);
                    bw.newLine();
                }
